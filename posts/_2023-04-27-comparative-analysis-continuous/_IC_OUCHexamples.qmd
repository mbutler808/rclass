...
-  **Exploratory plots**
  \begin{enumerate}
  -  Plot the data, with \code{ecomorph} as the independent variable and \code{lssd} as the dependent variable, which should produce a box-and-whiskers plot. 
```{r fig=T}
plot( ecomorph, lssd)
```
  ...
  -  Notice that there is a pesky \code{anc} category which does not have any observations. Get rid of it by running \code{factor} on it and then replot:
  \end{enumerate}
```{r }
...
\section*{ANOVA}
-  Run an ANOVA to test the hypothesis that \code{lssd} varies with \code{ecomorph} and interpret the results.
```{r }
