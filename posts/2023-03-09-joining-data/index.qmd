...
```{r}
geot <- geo[,"tarsusL"]  # geo is a matrix, select tarsusL column
geot$ecology <- LETTERS[1:nrow(geot)]   # made-up data, A:M
```
...
-   [**Mutating joins**](https://r4ds.had.co.nz/relational-data.html#mutating-joins): This is a typical __merge__ operation. It mutates because it adds on. A mutating join **combines variables from two tables**. It first matches observations by their keys, then copies across variables from one table to the other on the right side of the table (similar to `mutate()`). 
