...
Voila! This is a perfect example of a key - value pair. 

### Transparency

The transparency of colors is controlled by the aesthetic `alpha`, where 1 indicates 100% opacity (or 0 transparency).  
...
```{r}
p + geom_jitter(aes(col=Species, size = 3, alpha = 1/2)) +
  scale_color_manual(values=cols)
```
